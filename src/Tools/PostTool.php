...
class PostTool extends AbstractTool
{
     ...
     */
    #[McpTool(name: 'wp_update_post', description: 'Update existing post (partial - only provided fields change). Supports taxonomy term assignment.')]
        ...
        if ($taxonomies !== '') {
            ...
            if (is_array($taxData)) {
                foreach ($taxData as $taxonomy => $termIds) {
                    ...
                    $result = wp_set_object_terms($post_id, $termIds, $taxonomy);
                    if (is_wp_error($result)) {
